...
/* Insipired by openssl/crypto/x509/x509_cmp.c, which is buggy */
int JZ_X509_NAME_cmp(const X509_NAME *a, const X509_NAME *b)
{
    int i,j;
    ...
    for (i=sk_X509_NAME_ENTRY_num(a->entries)-1; i>=0; i--) {
        ...
        for (j = sk_X509_NAME_ENTRY_num(b->entries)-1; j>=0; j--) { 

             nb=sk_X509_NAME_ENTRY_value(b->entries,j);

             if (na->value->length != nb->value->length) continue;
             //if (na->value->length != nb->value->length || na->set != nb->set) continue;

             if (memcmp(na->value->data,nb->value->data, na->value->length)) continue;

             if (OBJ_cmp(na->object,nb->object)) continue;
            
             /* na == nb, exact match found */ 

             entry_matched = 1; 
             break;
       }

       if (entry_matched == 1) continue;
       else return 1; 
   }
...
{
	int rc = JZ_X509_NAME_cmp(X509_get_subject_name(cert), X509_REQ_get_subject_name(request));
